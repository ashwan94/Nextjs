...
export default function Signup() {
  ...
  // hanlder
  const idOnChnageHandler = useCallback((e) => {
    setId(e.target.value);
  }, []);
  const pwOnChnageHandler = useCallback((e) => {
    setPw(e.target.value);
  }, []);
  const nameOnChnageHandler = useCallback((e) => {
    setName(e.target.value);
  }, []);
  const locationOnChnageHandler = useCallback((e) => {
    setlocation(e.target.value);
  }, []);
  const fieldOnChnageHandler = useCallback((e) => {
  ...
  const goRegister = async () => {
    ...
    // 모든 항목이 빈 칸이 아닐 경우 DB 에 INSERT query 실행
    if (isDuple) {
      const res = await axios.post("/api/dbConnection", {
        url: "addData",
        id: id,
      ...
      // INSERT 성공했을 경우 1, 실패했을 경우 2 를 기반으로
      // if문 동작
      if (res.data == id) {
                ...
                <div className="flex -mx-3">
                  <div className="w-full px-3 mb-5">
                    ...
                    <div className="relative">
                      <select
                        onChange={locationOnChnageHandler}
