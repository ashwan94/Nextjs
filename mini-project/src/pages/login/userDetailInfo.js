...
export default function UserDetailInfo() {
  ...
  const id  = router.query.id // 이전 page 에서 넘겨받은 사용자 ID
  ...
  const introOnChangeHandler = useCallback((e) => {
    setIntro(e.target.value);
  }, []);

  // 성별 선택
  // 남성
  const clickMan = useCallback((e) => {
    setGender(e.target.id);
  }, []);
  // 여성
  const clickWoman = useCallback((e) => {
    setGender(e.target.id);
  }, []);

  // 닉네임 중복 여부 체크
  const nickNameRef = useRef();
  const [dupleBtn, setDupleBtn] = useState(false);
  const isDuple = async () => {
    if (nickName) {
  ...
  const doSignUp = async () => {
    ...
    if (nickName != "" && dupleBtn) {
      if (confirm) {
        ...
        // DB 에 UPDATE 성공 시 회원가입한 ID 를 응답받아 이를 근거로 예외처리
        if (res.data == id) {
          console.log("업데이트 결과 ==> ", res);
          alert("회원가입이 완료되었습니다!");
          localnStorage.removeItem("tempId"); // local 에 있는 임시 ID 삭제
          router.push("/login/signIn"); // 회원가입 완료 시 로그인 화면으로 이동
        } else {
          alert("문제가 발생했습니다. 고객센터에 문의주세요");
        }
      } else {
        alert("취소하셨습니다.");
      }
    }else{
      alert("닉네임 입력 또는 중복확인을 해주세요");
    }
  };
